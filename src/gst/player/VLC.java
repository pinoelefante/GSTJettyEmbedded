...
import gst.programma.OperazioniFile;
    ...
    public class VLC implements VideoPlayer {
    		...
    		if(Os.isWindows()){
    			try {
    				if(Advapi32Util.registryValueExists(WinReg.HKEY_LOCAL_MACHINE, "Software\\VideoLAN\\VLC", "")){
        				String dirVLC = Advapi32Util.registryGetStringValue(WinReg.HKEY_LOCAL_MACHINE, "Software\\VideoLAN\\VLC", "InstallDir");
		...
		public void SetVideoPlayerPath(String path){
			pathExe = path;
		}
		@Override
		public boolean canPlayVideo() {
			if(pathExe==null || pathExe.isEmpty() || !OperazioniFile.fileExists(pathExe))
				return false;
			return true;
		}
}
