...
public class ServerStart
{
	...
	{
		boolean UpdateEpisodi = true;

		if (args.length > 0)
		{
			Map<String, Boolean> opts = parseCMD(args[0]);
			/*
			 * if(!opts.get("startFromLauncher")) System.exit(-1); Boolean
			 * updates = opts.get("getUpdates"); if(updates!=null &&
			 * updates==true){ Sistema.getInstance().getCommandsUpdate(); //fare
			 * aggiornamento }
			 */
			if (opts.containsKey("aggiornaEpisodi"))
			{
				UpdateEpisodi = opts.get("aggiornaEpisodi");
			}
		}
